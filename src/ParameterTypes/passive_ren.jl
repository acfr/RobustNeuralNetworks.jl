...
Parameter struct to build a passive REN.
Note: ᾱ ∈ (0,1] is the upper bound on contraction rate.
"""
...
function PassiveRENParams{T}(
    ...
    if nu != ny
        error("Input and output must have the same dimension")
    end
    ...
    return PassiveRENParams{T}(nl, nu, nx, nv, ny, direct_ps, output_ps)

end

"""
    PassiveRENParams(nv, A, B, C, D; ...)

Alternative constructor for `PassiveRENParams` that initialises the
REN from a **stable** discrete-time linear system ss(A,B,C,D).

"""
function PassiveRENParams(
    nv::Int,
